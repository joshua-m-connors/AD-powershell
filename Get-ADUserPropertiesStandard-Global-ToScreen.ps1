...
If (Get-ADUser -Server $_ -Filter {samaccountname -eq $user} ) {
     Get-ADUser -Server $_ -Filter {samaccountname -eq $user} -Properties * | Select Name,GivenName,Surname,SamAccountName,Enabled,Company,Title,Manager,ObjectClass,ObjectGUID,SID,UserPrincipalName,DistinguishedName }
